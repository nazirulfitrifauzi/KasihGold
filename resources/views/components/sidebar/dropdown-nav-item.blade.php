<div x-data="{ {{$active}}:  @if(\Request::is($uri)) true @else false @endif }">
    <div class="relative flex items-center w-full space-x-2 text-white text-base font-semibold  hover:text-yellow-400 cursor-pointer"
    ...
    <div x-show.transition="{{$active}}">
        <ul x-transition:enter="transition-all ease-in-out duration-300" x-transition:enter-start="opacity-25 max-h-0"
            ...
            class="p-2 pr-0 mt-2 mr-0 mx-2 space-y-2 overflow-hidden text-sm font-medium  rounded-md rounded-r-none shadow-inner bg-gray-700"
