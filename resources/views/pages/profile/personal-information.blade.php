<x-general.card class="mt-2 bg-white shadow-lg">
    ...
    <div class="px-4 py-2">
        <x-form.basic-form wire:submit.prevent="saveReferrer">
            <x-slot name="content">
                <div class="mt-2 leading-4">
                    <div class="grid gap-2 lg:grid-cols-2 sm:grid-cols-1">
                        <x-form.input label="PG Code" wire:model="pgCode"/>
                        <x-form.input label="Introducer" wire:model="introducer"/>
                        <x-form.input label="Introducer Name" wire:model="introducerName"/>
                        <x-form.input label="Preferred Branch" wire:model="preferredBranch"/>
                    </div>
    ...
    <div class="px-4 py-2">
        <x-form.basic-form wire:submit.prevent="savePersonal">
            <x-slot name="content">
                <div class="mt-2 leading-4">
                    <div class="grid gap-2 lg:grid-cols-2 sm:grid-cols-1">
                        <x-form.input label="Name" wire:model="name"/>
                        <x-form.input type="email" label="Email Address" wire:model="email"/>
                        <x-form.input label="New IC" wire:model="ic"/>
                        <x-form.input label="Old IC" wire:model=""/>
                        <x-form.input label="Passport / Foreign ID" wire:model=""/>
                        <x-form.input label="Police / Army" wire:model=""/>
                        <x-form.input label="Phone No" wire:model="phone1"/>
                    </div>
